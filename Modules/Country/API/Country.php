...
class Country extends Model
{
	...
	public static function changeOrder(Request $request, Object $o_env)
	{
		...
		{
			$a_tmp = Country::whereIn('id', $request->order)->get()->pluck('id', 'country_id')->toArray();
			$a_parent_ids = array_keys($a_tmp);

			/**
			 *	let's keep things simple for now
			 *	respond with error if parent country filter has not been applied
			 */
			...
			if ($i_status == 201)
			{
				$a_tmp = Country::whereIn('id', $request->order)->get()->pluck('order', 'id')->toArray();
				$a_order_current = array_keys($a_tmp);

				/**
				 *	let's keep things simple for now
				 *	not all child countrys of this parent country were re-ordered in datatable country
				 */
			...
			if ($i_status == 201)
			{
				for ($i = 0; $i < $i_qty_sent; $i++)
				{
					 ...
					 */
					$o_country = Country::find($a_order_sent[$i]);
					$o_country->order = $i_order;
					$o_country->timestamps = FALSE;
					$o_country->save();
