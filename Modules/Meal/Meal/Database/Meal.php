...
class Meal extends Model
{
	...
	/**
	 * Calculate rating for each meal and save to DB
    ...
    private static function _calculateRating(Object $o_sql) : void
    {
		...
		for ($i = 0; $i < count($a_data); $i++)
		{
			...
			$o_meal->rating_last		= Carbon::now();

			$a_rating	= [];
			$a_rating	= self::_sumAllVotes($o_meal['vote'], $a_marks);
			$a_rating	= self::_setAverageTotal($a_rating, $a_elements, $i_mark_max, $i_mark_min);

			$i_tmp		= $o_meal['opinion']->count();
			$a_rating['overall']['latest_opinion'] = '';
			for ($j = $i_tmp; $j > 0; $j--)
			{
				if (!empty($o_meal['opinion'][$j]['description']))
				{
					$a_rating['overall']['latest_opinion'] = $o_meal['opinion'][$j]['description'];
					break;
				}
			}

			$o_meal->rating_all		= $a_rating['overall']['percent'];
			$o_meal->rating_min		= $a_rating['lowest']['elements'];
			unset($a_rating['overall']['percent']);
			unset($a_rating['lowest']);

			$o_meal->rating_info		= json_encode($a_rating);

			$b_send		= TRUE;
			$b_send		= $b_send && (
							/**
							 * Meal overall rating (in percent) is lower than the allowed threshold
							 */
							$o_meal->rating_all > -1
							&& $o_meal	->rating_all < $i_lowest_allowed
							||
							/**
							 * One of the meal's elements rating (in percent) is lower than the allowed threshold
							 */
							$o_meal->rating_min > -1
							&& $o_meal	->rating_min < $i_lowest_allowed
						);
			/**
			 * Qty of active complaints already sent for this meal
			 * Responses not received so far
			 * Also update complaints counter, just in case it's outdated
			 */
			$o_meal->complaint_qty = Complaint::select('id')->wherePublished(1)->whereMealId($o_meal->id)->count();
			$b_send		= $b_send && ($o_meal->complaint_qty < 1);
			/**
			 ...
			 */
			$b_send		= $b_send && ($o_meal->complaint_delay < Carbon::now());
			/**
			...
			/**
			 * At least this much unique opinions (votes) were cast for meal's elements
			 */
			...
			if ($b_send)
			{
				...
				Complaint::sendIt($o_meal, $s_tmp);
				$a_update['complaint_qty']		= $o_meal->complaint_qty;
			}
			 ...
			 */
			$a_tmp = array_intersect_key($o_meal->toArray(),array_flip($a_update));
	 ...
	 *
	 * @param String	$s_format_set	Format describing mealment of information
	 ...
	 * for each element and for the meal in general
	 *
	 * @param Array		$a_rating		All votes organised per element and per meal
					...
					$a_res['element'][] =
						[	'percent'		=> round($i_percent),
							...
							'description'	=> self::_getMarkDescription(
								...
								Meal::formatNumber($i_tmp, 1),
								Meal::formatNumber($i_sum_votes),
								]
							),
						]
							;
...
#			$i_tmp = $i_mark_max * ($i_overall / 100);
			...
			$a_res['overall']['details'] = self::_getMarkDescription(
						...
						[
						Meal::formatNumber($i_tmp, 1),
						Meal::formatNumber($i_mark_max),
						Meal::formatNumber($i_overall_votes),
						]
					);
		}
		return $a_res;
	 ...
	 *
	 * @param Array		$a_votes		All votes casted for this meal
